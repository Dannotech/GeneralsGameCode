...
void turf_war::Logic( float DeltaTime )
{
    ...
    //
    // Process Deja Console.
    //

    char Buffer[1024];
    DEJA_CONSOLE_READ( Buffer, sizeof(Buffer) );
    if( Buffer[0] != 0 )
    {
        if( strstr( Buffer, "help" ) )
        {
            DEJA_CONSOLE_WRITE( "TurfWar - Deja Insight Demo" );
            DEJA_CONSOLE_WRITE( "---------------------------" );
            DEJA_CONSOLE_WRITE( "lag - Toggle the bug that causes framerate lag to demonstrate context." );
        }
        else
        if( strstr( Buffer, "lag" ) == Buffer )
        {
            g_BugLag = !g_BugLag;
            DEJA_CONSOLE_WRITE( "Lag = %s", g_BugLag ? "On" : "Off" );
        }
        else
        {
            DEJA_CONSOLE_WRITE( "ERROR: Unknown command." );
        }
    }
...
void turf_war::Render( void )
{
    ...
    // Test code to add some color to the background.
    {
        ...
        for( int i=0; i<5; i++ )
        {
            ...
            c[i] = Graphics->PackColor( color(r,g,b,255) );
            v[i] = s[i] + d[i] * 100.0f;
        }
    ...
    // Render diagnostics.
    {
        if( !(DEJA_LIB_CONNECTED()) )
        {
            ...
            if( *pError )
            {
                Engine->sprintf( Buffer, sizeof(Buffer), 
                                 "Not Connected - DEJA_LAST_ERROR: \"%s\"",
                                 pError );
            }
            else
            {
                Engine->sprintf( Buffer, sizeof(Buffer), "Disconnected" );
            }
