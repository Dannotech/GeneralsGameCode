...
void ComputeCellVerts( cell& c )
{
    ...
    for( int s=0; s<6; s++ )
    {
        ...
        c.m_Lines[iLine+0].Color    = 0xc0c0c0c0;
        c.m_Lines[iLine+1].Color    = 0xc0c0c0c0;
        iLine += 2;
...
void ComputeCellNormals( cell& c )
{
    ...
    for( int iVert=0; iVert<(int)(sizeof(c.m_Triangles)/sizeof(c.m_Triangles[0])); iVert+=3 )
    {
...
bool arena::Intersects( const vector3& Point )
{
    // Setup some constants for the arena.
    float   Side     = HEX_SIDE;
    float   Sin30    = 0.5f;
...
bool arena::Intersects( const vector3& Start, const vector3& End )
{
    DEJA_CONTEXT( "arena::Intersects" );

    vector3 Delta   = Start - End;
    float   Length  = Delta.GetLength();
    int     nPoints = (int)(Length / HEX_SIDE);
    nPoints = MAX( nPoints, 1 );

    // Simulate a ray intersection test that takes a long time
    float Delay = 0.0001f;
    float Time  = Engine->GetRealTime();
    while( (Engine->GetRealTime() - Time) < Delay ) ;

    // Test points along the ray for intersection
    for( int i=0; i<=nPoints; i++ )
    {
        float t = (float)i / (float)nPoints;
        vector3 p = Start + Delta * t;
        if( Intersects( p ) )
            return( true );
    }
