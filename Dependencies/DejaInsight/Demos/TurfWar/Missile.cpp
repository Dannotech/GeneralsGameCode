...
void SystemPrep_Missile( void )
{
    ...
    DEJA_ENUM_ALIAS( missile::state, missile::MISSILE_STATE_TARGET_DESTROYED, "TARGET_DESTROYED" );
...
void missile::UpdateTargetPos( void )
{
    ...
    else
    {
        ...
        m_TargetID = -1;
        m_State    = MISSILE_STATE_TARGET_DESTROYED;
...
int missile::GetBestTarget( const char* pTypeName )
{
    DEJA_CONTEXT( "missile::GetBestTarget" );

    vector3 p = GetPosition();
    int iType = ObjMgr.TypeNameToIndex( pTypeName );

    // Setup to get all the objects
    target* pTargets   = NULL;
    int     nArraySize = 0;
    int     nTargets   = 0;

    // Get all the objects
    ObjMgr.ResetTypeLoop( iType );
    object* pTarget;
    while( (pTarget = ObjMgr.GetNextInType( iType )) != NULL )
    {
        // Check for arena collision from missile to target
        ObjMgr.ResetTypeLoop( ArenaTypeData.iType );
        arena* pArena = (arena*)ObjMgr.GetNextInType( ArenaTypeData.iType );
        if( !pArena || !pArena->Intersects( p, pTarget->GetPosition() ) )
        {
            // Check if we need to increase storage
            if( nTargets >= nArraySize )
            {
                // Increase storage
                nArraySize++;
                target* pNew = new target[nArraySize];
                ...
                // Add the new target
                pTargets[nTargets].ID     = pTarget->GetID();
                pTargets[nTargets].Weight = 0.0f;
                nTargets++;
            }
        }
    }

    // Evaluate the weight for all potential targets
    for( int i=0; i<nTargets; i++ )
    {
        // Compute weight from distance to other potential targets
        for( int j=0; j<nTargets; j++ )
        {
            ...
            {
                // Get the 2 objects
                object* pObject1 = ObjMgr.GetObjByID( pTargets[i].ID );
                object* pObject2 = ObjMgr.GetObjByID( pTargets[j].ID );

                // Only continue if both objects are valid
                if( pObject1 && pObject2 )
                {
                    if( g_BugLag )
                    {
                        // Can the objects see each other?
                        ObjMgr.ResetTypeLoop( ArenaTypeData.iType );
                        arena* pArena = (arena*)ObjMgr.GetNextInType( ArenaTypeData.iType );
                        if( !pArena || !pArena->Intersects( pObject1->GetPosition(), pObject2->GetPosition() ) )
                        {
                            // Get the distance between the targets
                            vector3 Delta = pObject1->GetPosition() - pObject2->GetPosition();
                            float Distance = Delta.GetLength();
                            Distance = MAX( 1.0f, Distance );

                            // Increase the weight according to the distance
                            pTargets[i].Weight += 1.0f / Distance;
                        }
                    }
                    else
                    {
    ...
    for( int i=0; i<nTargets; i++ )
    {
        // Is this target better?
        if( pTargets[i].Weight > BestWeight )
        {
            // Save current best
            BestTarget = i;
            BestWeight = pTargets[i].Weight;
        }
    }

    // Return the best target found
    if( BestTarget != -1 )
        return pTargets[BestTarget].ID;
    else
        return -1;
}
...
void missile::OnLogic( float Time )
{
    ...
    ObjMgr.ResetTypeLoop( ArenaTypeData.iType );
    arena* pArena = (arena*)ObjMgr.GetNextInType( ArenaTypeData.iType );
    if( pArena && pArena->Intersects( GetPosition() ) )
    {
    ...
    case MISSILE_STATE_TARGET_DESTROYED:
        LogicTargetDestroyed( Time );
        break;
...
void missile::LogicTargetDestroyed( float Time )
{
    DEJA_CONTEXT( "missile::LogicTargetDestroyed" );
